$ChildPath = '\Evidencias\'+ $Client+'\AnulacionCompra'
...
Describe 'Validacion de parametros invalidos' {
    ...
    Write-Host 'Generando compra para anular'
    ...
    $Cases.Where( { $_ -ne $null }) | ForEach-Object {
        It $("solicitud debe retornar respuesta con codigo 400 si $($_.Parameter) $($_.Description)") {
            ...
            $Request.AuthNumber = $Payments[1].AuthNumber
    ...
    $Payments.Where( { $_ -ne $null }) | ForEach-Object {
        ...
        Describe "Anulación de compra Casos Exito, Todos los tipos de identificación: $($_.DocType)-$($_.DocNumber)" {
            ...
            try {
                ...
                It "Microservices: MessageType correponde con una Transaccion de Tipo 0200" {
                    if ($TransactionMicroservices.message_type -eq '0200') {
                        $message_type = 'True'
                    }
                ...
                It "Microservices: AccountType registrada en la transacción corresponde con la cuenta a la que se retorna el dinero" {
                    if ($Request.AccountType -eq $TransactionMicroservices.to_account ) {
    ...
    $Payments.Where( { $_ -ne $null }) | ForEach-Object {
        #$TransactionMongo = Get-DataMongo -Value1 $Config.Channel -Value2 $_ -Parameter 'QueryReversoTipos'
        Describe "Anulación de compra Casos Exito, con los bolsillos (80, 81, 82 y 83): $($_.DocType)-$($_.DocNumber)" {
    ...
    $Payments.Where( { $_ -ne $null }) | ForEach-Object {
        #$TransactionMongo = Get-DataMongo -Value1 $Config.Channel -Value2 $_ -Parameter 'QueryReversoTipos'
        Describe "Anulación de compra Casos Exito, con los CardAcceptorId configurados: $($_.DocType)-$($_.DocNumber)" {
    ...
    Describe 'Excepcion: Realizar Anulación de compra con un DocType erroneo' {

        Write-Host 'Generando compra para anular'
        $Payments = New-PaymentTransaction -Token $Token -PaymentTest 4
        Write-Host 'Compra generada'

        $IdCard = $Payments.IdCard
        $Pan = $Payments.Pan

        #Construccion del Request
        $Request = New-RefundPaymentRequest
        $Request.AuthNumber = $Payments.AuthNumber
        $Request.DocType = "HJ"
        $Request.DocNumber = $Payments.DocNumber
        $Request.AccountType = $Payments.AccountType
        $Request.Amount = $Payments.Amount
        $Request.Tags.CardAcceptorId = $Payments.CardAcceptorId
        $DeltaLedgerBalance = $Payments.SaldoCompra
        $Request1 = $Request | ConvertTo-Json
    ...
    Describe 'Excepcion: Realizar Anulación de compra con un valor de transacción distinto al original' {
    ...
    Describe 'Excepcion: Realizar Anulación de compra con un CardAcceptorId configurado para el cliente, sin permisos' {
    ...
    Describe 'Excepcion: Realizar Anulación de compra con un CardAcceptorId que no exista' {
    ...
    Describe 'Excepcion: Realizar Anulación de compra con un pan diferente al del cliente' {
    ...
    Describe 'Excepcion: Realizar Anulación de compra sin enviar el pan del cliente' {
    ...
    Describe 'Excepcion: Realizar Anulación de compra con un CustomerGroup que no exista en la BD' {
    ...
    Describe 'Excepcion: Realizar Anulación de compra con un AuthNumber que no existe' {
        ...
        $Payments.Where( { $_ -ne $null }) | ForEach-Object {
            ...
            #Construccion del Request
            $Request = New-RefundPaymentRequest
            $Request.AuthNumber = Get-Random -Minimum 100000 -Maximum 999999
            $Request.DocType = $_.DocType
            $Request.DocNumber = $_.DocNumber
            $Request.AccountType = $_.AccountType
            $Request.Amount = $_.Amount
    ...
    Describe 'Excepcion: Realizar Anulación de compra con un DocNumber diferente al del cliente' {
    ...
    Describe 'Excepcion: Realizar Anulacion de compra con un Bolsillo diferente al del cliente' {
